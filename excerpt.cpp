        ...
        std::vector<fp_t> &coefficients) // storage where to put the coefficients; size should exceed P
{
    ...
    assert(N_clustered_roots!=1); assert(N_multiple_roots!=1); assert(n_simple_roots>=0 && P<=4);
    ...
    switch (P)
    {
        ...
        case 2:
        {
            ...
            coefficients[1]=-re-im; coefficients[0]=re*im;
        ...
        case 3:
        {
            if (N_pairs_of_complex_roots==1) // one real root
            {
                ...
                coefficients[0]=-im*u; coefficients[2]=re-u; coefficients[1]=fma(-re,u,im); // im-re*u;
                return 1;
            }
            ...
            coefficients[2]=-re-im-u; coefficients[0]=-re*im*u;
            v=pr_product_difference(re,im,-re,u); coefficients[1]=fma(im,u,v); // re*im+re*u+im*u=im*u+(re*im-(-re*u));
        ...
        case 4: // DEN DEBUG: check it carefully
        {
            ...
            {
                re=rnr(rng); im=rnr(rng);
                im=re*re+im*im; re*=static_cast<fp_t>(-2.0L); // irreducible quadratic polynomial is (x^2 + re*x + im)
                u=rnr(rng); v=rnr(rng);
                v=u*u+v*v; u*=static_cast<fp_t>(-2.0L); // irreducible quadratic polynomial is (x^2 + u*x + v)
                // multiply both irreducible quadratics
                coefficients[0]=im*v; coefficients[1]=re*v+im*u; coefficients[2]=im+re*u+v; coefficients[3]=re+u;
                return 0;
            }
            else if (N_pairs_of_complex_roots==1) // two real roots
            {
                re=rnr(rng); im=rnr(rng); roots[0]=u=rnr(rng);
                im=re*re+im*im; re*=static_cast<fp_t>(-2.0L); // irreducible quadratic polynomial is (x^2 + re*x + im); multiply it by (x-u)
                ...
                root_mid_sweep=-u-v; v*=u; u=root_mid_sweep; // two-root quadratic polynomial is (x^2 + u*x + v)
                // multiply irreducible and reducible quadratics
                coefficients[0]=im*v; coefficients[1]=re*v+im*u;
                coefficients[2]=im+re*u+v; coefficients[3]=re+u;
                return 2;
            }
            ...
            root_mid_sweep=-re-im; im*=re; re=root_mid_sweep; // now we have the 1.st quadratic polynomial: x^2 + x*re + im
            root_mid_sweep=-u-v; v*=u; u=root_mid_sweep; // now we have the 2.nd quadratic polynomial: x^2 + x*u + v
            coefficients[0]=im*v; coefficients[1]=re*v+im*u; coefficients[2]=re*u+im+v; coefficients[3]=re+u; return 4;
        } // P=4
    } // switch (P)
...
// (roots_to_check) and (roots_ground_truth)
{
    ...
    for (int i=0; i<N_roots_ground_truth; ++i)
    {
        ...
        for (int j=0; j<N_roots_to_check; ++j)
        {
            ...
            deviation_min_for_this_root=deviation<deviation_min_for_this_root ? deviation : deviation_min_for_this_root;
        }
